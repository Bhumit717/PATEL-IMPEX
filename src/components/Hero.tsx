...
const Hero = () => {
  return (
    <main className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-gradient-to-br from-gray-900 via-black to-gray-900" role="banner" aria-label="Patel Impex - Global Import Export Company Hero Section">
      ...
      <div className="relative container mx-auto px-4 z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            ...
            <div className="inline-flex items-center px-6 py-3 bg-gray-800/80 border border-ai-primary/30 rounded-full min-h-[44px]" role="banner" aria-label="Trust indicator">
            ...
            <section className="grid grid-cols-3 gap-8 pt-8" aria-label="Company statistics and achievements">
              {[
                { number: "1120+", label: "Global Clients", description: "Import export businesses we've served" },
                ...
                <div key={index} className="text-center group cursor-pointer min-h-[80px] p-2" role="article" aria-label={`${stat.description}: ${stat.number}`} tabIndex={0}>
          ...
          <section className="grid grid-cols-1 gap-6" aria-label="Key features and services">
            {[
              ...
              { 
                ...
                desc: "Scale your import and export businesses internationally with data-driven market insights", 
              ...
              <article key={index} className="group cursor-pointer transform hover:scale-105 transition-all duration-300" role="article" aria-label={`${item.title} service feature`} tabIndex={0}>
                <div className={`${item.bgColor} p-8 rounded-2xl border border-ai-primary/20 hover:border-ai-primary/40 transition-all duration-300 hover:shadow-lg hover:shadow-ai-primary/20 focus:ring-2 focus:ring-ai-primary focus:outline-none min-h-[200px]`}>
                  <div className={`w-16 h-16 bg-gradient-to-r ${item.color} rounded-2xl flex items-center justify-center mb-6`} aria-hidden="true">
      ...
      {/* Improved scroll indicator with better touch target */}
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20" role="navigation" aria-label="Scroll indicator">
        <button 
          className="flex flex-col items-center space-y-2 p-4 -m-4 min-h-[44px] min-w-[44px] focus:outline-none focus:ring-2 focus:ring-ai-primary rounded-lg"
          onClick={() => window.scrollTo({ top: window.innerHeight, behavior: 'smooth' })}
          aria-label="Scroll down for more content"
        >
          <div className="w-6 h-10 border-2 border-ai-primary rounded-full flex justify-center relative" aria-hidden="true">
