...
const Navigation = () => {
  ...
  return <header className={`fixed w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-2xl' : 'bg-white'}`} role="banner">
      ...
      <nav className="bg-white/95 backdrop-blur-sm relative" role="navigation" aria-label="Main navigation">
          ...
          <div className="flex justify-between items-center h-16 md:h-20">
            <Link to="/" className="flex items-center group" aria-label="Patel Impex - Home page">
              <div className="relative flex items-center">
                <div className="w-10 h-10 md:w-14 md:h-14 rounded-full overflow-hidden shadow-lg group-hover:shadow-xl transition-all duration-500 mr-2 md:mr-3">
                  <img src="/lovable-uploads/9441ee54-1129-4a42-899b-fae2056a0aec.png" alt="Patel Impex Logo" className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500 bg-white p-1" />
                </div>
            ...
            <div className="hidden md:block" role="menubar" aria-label="Main navigation menu">
              <div className="ml-10 flex items-baseline space-x-8">
                {navItems.map((item, index) => <Link key={item.name} to={item.href} className={`relative px-4 py-3 text-sm font-bold transition-all duration-500 group focus:ring-2 focus:ring-blue-500 focus:outline-none rounded ${location.pathname === item.href ? 'text-blue-600 scale-110' : 'text-slate-700 hover:text-blue-600 hover:scale-105'}`} style={{
                animationDelay: `${index * 0.1}s`
                    ...
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                      <div className="absolute top-0 left-1/4 w-1 h-1 bg-blue-400 rounded-full animate-float" style={{
                    animationDelay: '0s'
                  }}></div>
                      <div className="absolute top-0 right-1/4 w-1 h-1 bg-teal-400 rounded-full animate-float" style={{
                    animationDelay: '0.5s'
                  }}></div>
                    </div>
                  </Link>)}
            ...
            <div className="md:hidden">
              <button onClick={() => setIsOpen(!isOpen)} className="text-slate-700 hover:text-blue-600 transition-all duration-300 p-2 rounded-lg hover:bg-blue-50 group focus:ring-2 focus:ring-blue-500 focus:outline-none" aria-expanded={isOpen} aria-controls="mobile-menu" aria-label={isOpen ? "Close navigation menu" : "Open navigation menu"}>
                <div className="relative">
                  {isOpen ? <X size={28} className="group-hover:rotate-90 transition-transform duration-300" aria-hidden="true" /> : <Menu size={28} className="group-hover:scale-110 transition-transform duration-300" aria-hidden="true" />}
                  <div className="absolute -inset-1 bg-blue-400 rounded-lg opacity-0 group-hover:opacity-20 transition-opacity duration-300" aria-hidden="true"></div>
                </div>
              </button>
            </div>
          </div>
        ...
        <div id="mobile-menu" className={`md:hidden transition-all duration-500 transform origin-top ${isOpen ? 'max-h-96 opacity-100 scale-y-100' : 'max-h-0 opacity-0 scale-y-95 overflow-hidden'}`} role="menu" aria-label="Mobile navigation menu">
