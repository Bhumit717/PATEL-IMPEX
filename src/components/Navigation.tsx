        ...
        <div className="container mx-auto flex justify-between items-center text-sm relative z-10">
          ...
          <div className="hidden md:flex items-center space-x-4" role="text" aria-label="Company certification">
            <div className="flex items-center space-x-2">
              <Sparkles size={14} className="text-yellow-500 animate-pulse" aria-hidden="true" />
...
/>
              ...
              <div className="ml-10 flex items-baseline space-x-8">
                {navItems.map((item, index) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    className={`relative px-4 py-3 text-sm font-bold transition-all duration-500 group focus:ring-2 focus:ring-blue-500 focus:outline-none rounded ${
            ...
            <div className="md:hidden">
              <button
                ...
                className="text-slate-700 hover:text-blue-600 transition-all duration-300 p-2 rounded-lg hover:bg-blue-50 group focus:ring-2 focus:ring-blue-500 focus:outline-none"
              ...
              >
                <div className="relative">
                  {isOpen ? (
                    <X size={28} className="group-hover:rotate-90 transition-transform duration-300" aria-hidden="true" />
                  ) : (
                    <Menu size={28} className="group-hover:scale-110 transition-transform duration-300" aria-hidden="true" />
                  )}
        ...
        <div 
          id="mobile-menu" 
          className={`md:hidden transition-all duration-500 transform origin-top ${isOpen ? 'max-h-96 opacity-100 scale-y-100' : 'max-h-0 opacity-0 scale-y-95 overflow-hidden'}`}
          role="menu"
          ...
          <div className="bg-white border-t border-blue-100 px-4 pt-4 pb-6 space-y-2 shadow-2xl relative overflow-hidden">
            ...
            {navItems.map((item, index) => (
              <Link
                key={item.name}
                to={item.href}
                className={`block px-4 py-4 text-base font-bold rounded-2xl transition-all duration-300 relative group focus:ring-2 focus:ring-blue-500 focus:outline-none ${
                  location.pathname === item.href
